...
export const CoursesList: React.FC = () => {
  ...
  const {isPending, isError, data, isFetching, isPlaceholderData} = useQuery<PaginatedDTO<CourseDTO>>({
...
return (
  <TableContainer component={Paper}>
    <Table>
      ...
      <TableBody>
        ...
        { !isPending && data?.data?.map((row) => (
            <TableRow key={row.title}>
              ...
              <TableCell>{formatSchedule(row.schedule)}</TableCell>
            </TableRow>
          ))
        }
      ...
      <TableFooter>
        <TableRow>
          <TablePagination count={data?.total || 0} page={data?.page || 0} rowsPerPage={data?.pageSize || 10} onPageChange={() => {
          }}/>
        </TableRow>
      </TableFooter>
    </Table>
